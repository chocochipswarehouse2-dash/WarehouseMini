...
import { PengirimanStoreTab } from './Pengiriman/PengirimanStoreTab';
import { PengirimanPaketTab } from './Pengiriman/PengirimanPaketTab';
...
export const PengirimanView: React.FC<PengirimanViewProps> = ({
  ...
  return (
    <div className="p-2 sm:p-4 space-y-4 max-w-5xl mx-auto pb-24">
      ...
      {instrumentTab === 'store' && (
        <PengirimanStoreTab session={session} onShowToast={onShowToast} />
      )}

      {instrumentTab === 'paket' && (
        <PengirimanPaketTab session={session} onShowToast={onShowToast} />
      )}
    </div>
  );
};
