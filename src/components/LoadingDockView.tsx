...
export function LoadingDockView({ session, onShowToast }: any) {
  const userIsAdmin = isSuperadmin(session);
  
  const tabs = [
    { id: 'penerimaan', permissionKey: 'tab_ops_loading_penerimaan', label: 'Penerimaan' },
    { id: 'pengiriman', permissionKey: 'tab_ops_loading_pengiriman', label: 'Pengiriman' }
  ].filter(t => userIsAdmin || hasPermission(session, t.permissionKey));

  const [activeTab, setActiveTab] = useState<'penerimaan' | 'pengiriman'>(
    (tabs.length > 0 ? tabs[0].id : 'penerimaan') as 'penerimaan' | 'pengiriman'
  );
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-[#0a0f1c]">
      ...
      <div className="p-2 sm:p-4 pb-0 shrink-0">
        <div className="bg-slate-100/90 dark:bg-[#09090b]/90 p-1 sm:p-1.5 rounded-xl sm:rounded-2xl border border-slate-200/80 dark:border-slate-800/80 shadow-xs max-w-md">
