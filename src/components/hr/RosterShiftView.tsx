...
export const RosterShiftView: React.FC<RosterShiftViewProps> = ({ session, onShowToast }) => {
  ...
  return (
    <div className="space-y-3 animate-in fade-in duration-200">
      ...
      {/* FILTERS & SEARCH BAR */}
      <div className="p-5 rounded-3xl bg-white dark:bg-[#131d31] shadow-sm border border-slate-200 dark:border-slate-800 space-y-2">
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-900/60 border-b border-slate-200 dark:border-slate-800 text-slate-500 font-black uppercase text-[10px] tracking-wider">
                  <th className="py-3.5 px-2">Tanggal</th>
                  <th className="py-3.5 px-2">Staf & NIK</th>
                  <th className="py-3.5 px-2">Divisi</th>
                  <th className="py-3.5 px-2">Shift Kerja</th>
                  <th className="py-3.5 px-2">Jam Masuk - Pulang</th>
                ...
                {filteredRoster.map((r) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-2 font-mono font-bold text-slate-600 dark:text-slate-300 whitespace-nowrap">
                        {isLibur ? '-' : `${r.jam_masuk || '08:00'} - ${r.jam_pulang || '17:00'}`}
                      </td>
                      <td className="py-3 px-2 text-slate-500 italic max-w-xs truncate">
                        {r.keterangan || '-'}
                      </td>
                    </tr>
                  );
