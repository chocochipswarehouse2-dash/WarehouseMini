...
import { Store, Package, Send, ArrowRightLeft, Layers } from 'lucide-react';
...
import { PengirimanDummyTab } from './PenerimaanBarang/PengirimanDummyTab';
...
export const PenerimaanBarangView: React.FC<PenerimaanBarangViewProps> = ({
  ...
  const [instrumentTab, setInstrumentTab] = useState<'mutasi_store' | 'penerimaan_paket' | 'pengiriman'>(
    'mutasi_store'
  );
  ...
  return (
    <div className="p-2 sm:p-4 space-y-4 max-w-5xl mx-auto pb-24">
      ...
      <div className="bg-slate-100/90 dark:bg-[#09090b]/90 p-1 sm:p-1.5 rounded-xl sm:rounded-2xl border border-slate-200/80 dark:border-slate-800/80 shadow-xs">
        <div className="grid grid-cols-3 sm:flex sm:items-center gap-1 sm:gap-1.5">
          <button
            ...
            className={`py-2 sm:py-2.5 px-2.5 sm:px-3 text-xs sm:text-sm font-bold flex items-center justify-center gap-1.5 sm:gap-2 rounded-lg sm:rounded-xl transition-all duration-200 cursor-pointer select-none ${
          ...
          <button
            type="button"
            id="tab-instrument-pengiriman-dummy"
            onClick={() => setInstrumentTab('pengiriman')}
            className={`py-2 sm:py-2.5 px-2.5 sm:px-3 text-xs sm:text-sm font-bold flex items-center justify-center gap-1.5 sm:gap-2 rounded-lg sm:rounded-xl transition-all duration-200 cursor-pointer select-none ${
              instrumentTab === 'pengiriman'
      ...
      {instrumentTab === 'pengiriman' && <PengirimanDummyTab />}
    </div>
  );
};
