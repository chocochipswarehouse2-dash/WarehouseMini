...
// ------------------------------------------------------------
// MODUL QUALITY CONTROL (QC) - FETCH, SAVE, DELETE
// ------------------------------------------------------------

export async function fetchQcReportsFromSupabase(): Promise<QcReport[]> {
  try {
    const data = await supabaseFetch<QcReport[]>('qc_reports', 'GET', undefined, 'order=created_at.desc');
    if (data && Array.isArray(data)) {
      try {
        localStorage.setItem('wms_local_qc_reports', JSON.stringify(data));
      } catch {}
      return data;
    }
  } catch (err) {
    console.warn('Gagal memuat laporan QC dari Supabase, beralih ke cache lokal:', err);
  }
...
export async function saveQcReportToSupabase(report: QcReport): Promise<QcReport> {
  let savedReport: QcReport = { ...report };

  // Generate report_no if not present
  if (!savedReport.report_no) {
    const dateStr = new Date().toISOString().slice(0, 10).replace(/-/g, '');
    const rand = Math.floor(100 + Math.random() * 900);
    savedReport.report_no = `QC-${dateStr}-${rand}`;
  }

  // Try saving to Supabase qc_reports table
  try {
    const payload = {
      ...savedReport,
    ...
    const res = await supabaseFetch<QcReport[]>('qc_reports', 'POST', [payload]);
    if (res && Array.isArray(res) && res.length > 0) {
      savedReport = { ...res[0] };
    }
  } catch (err) {
    console.warn('Gagal menyimpan laporan QC ke Supabase (akan disimpan di cache lokal):', err);
  }

  // Sync to local cache
  try {
    const cachedStr = localStorage.getItem('wms_local_qc_reports');
    const list: QcReport[] = cachedStr ? JSON.parse(cachedStr) : [];
    const filtered = list.filter((r) => r.report_no !== savedReport.report_no);
...
export async function deleteQcReportFromSupabase(reportNoOrId: string | number): Promise<boolean> {
  try {
    const query = typeof reportNoOrId === 'number'
      ? `id=eq.${reportNoOrId}`
      : `report_no=eq.${encodeURIComponent(String(reportNoOrId))}`;
    await supabaseFetch('qc_reports', 'DELETE', undefined, query);
  } catch (err) {
    console.warn('Gagal menghapus laporan QC dari Supabase:', err);
  }
  ...
  try {
    ...
    if (cachedStr) {
      ...
      const filtered = list.filter(
        (r) => r.id !== reportNoOrId && r.report_no !== String(reportNoOrId)
      );
      localStorage.setItem('wms_local_qc_reports', JSON.stringify(filtered));
    }
  } catch {}
