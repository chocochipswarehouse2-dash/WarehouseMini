---
description: Standard workflow rules for Git pull on start, Supabase migration synchronization, and build & auto-deploy on completion.
---
...
- **Synchronize Column Mapping**: Ensure field mappings (e.g., `no_pesanan`, `nama_penerima`, `alamat`, `jasa_kirim`, etc.) in the migration scripts match the current Supabase tables.
- **Maintain Schema Consistency**: Keep DDL and migration files mutually consistent.

## 3. End of Session / Post-Task (Build & Auto-Deploy)
...
2. **Commit & Push**: Automatically stage, commit with a descriptive conventional commit message, and push to GitHub:
   ```bash
   git add <modified-files>
   git commit -m "<type>: <concise description of changes>"
   git push origin main
   ```
3. **Trigger Deployment**: Pushing to `main` automatically triggers GitHub Actions (`deploy.yml`) and/or Vercel deployment. Always inform the user that the push and deploy have been initiated.
